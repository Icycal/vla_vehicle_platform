...
最终 `/cmd_vel` 保持零且未启动 `wheeltec_robot_node`。

截至 2026-08-07，C++ `vehicle.dataset.v1` Exporter、隔离 Observation Rosbag Replay 和原始 Episode
到 SmolVLA Shadow 的回放链路已验证。配置驱动的 LeRobot v3 Dataset 转换器也已完成，
支持多 Episode、前视图像、状态有效性掩码、Ackermann 二维动作和源 FPS 一致性检查。
